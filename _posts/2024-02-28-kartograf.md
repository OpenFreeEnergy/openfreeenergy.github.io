Should I make this or that molecule? Relative Binding Free Energies - Setup: About mapping atoms with Kartograf.
==================================================================================================================
...
Recently we published an article in JCTC about Kartograf, a new atom mapping
...
differently. The RBFE protocol, which was developed by the Perses team and which
...
## Atom Mappers
Existing solutions in this area (for example Lomap, fesetup, and timemachine)
...
Dalke here. In general they can be said to first work on the 2D structures (
...
thanks to conversations with old colleagues. This alternative approach is
...
problem, where the distance matrix between the two small molecules form a
weighted bipartite graph, which we solve using the linear sum assignment
...
You can install the package with PyPi:
```bash
pip install kartograf
```

Or you can simply install ith with conda:
```bash
...
`openfe plan-rbfe-network` command can take a yaml file to customise the
algorithms used.
